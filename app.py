    ...
    if request.method == 'POST':
        ...
        if(my_tasks):
            ...
            if int(my_tasks[-1].content) == randomZahl:
                return render_template('index.html', richtig=richtig, aktuelleZahl=aktuelleZahl, versuche=versuche)
            elif int(my_tasks[-1].content) > randomZahl:
                return render_template('index.html', tiefer=tiefer, aktuelleZahl=aktuelleZahl, versuche=versuche)
            elif int(my_tasks[-1].content) < randomZahl:
                return render_template('index.html', hoeher=hoeher, aktuelleZahl=aktuelleZahl, versuche=versuche)
        else:
